			...
			if (resource === 'post') {
				if (operation === 'create') {
					for (let i = 0; i < length; i++) {
						...
						if (contentFormat === 'html') {
							...
							post.mobiledoc = JSON.stringify(content);
						}
						...
						responseData = await ghostApiRequest.call(this, 'POST', '/admin/posts', { posts: [post] });
				...
				if (operation === 'update') {
					for (let i = 0; i < length; i++) {			
						...
						responseData = await ghostApiRequest.call(this, 'PUT', `/admin/posts/${postId}`, { posts: [post] });
